...
if __name__ == '__main__':
    ...
    parser.add_argument('--latest-untracked', action='store_true',
                        help='Date du dernier article sans métriques')
    args = parser.parse_args()

    if args.latest_untracked:
        from metrics_db import get_latest_without_metrics
        date = get_latest_without_metrics()
        if date:
            title, themes = resolve_article_metadata(date)
            result = {"date": date, "title": title, "themes": themes}
        else:
            result = {"date": None, "message": "Tous les articles ont des métriques"}
